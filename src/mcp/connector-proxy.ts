...
const failedAuthAttempts = new Map<string, { count: number; firstFailureAt: number }>();
...
const publicBaseUrl = normalizeBaseUrl(requiredEnv('MACROFACTOR_CONNECTOR_PUBLIC_BASE_URL'));
const publicPath = normalizePath(optionalEnv('MACROFACTOR_CONNECTOR_PUBLIC_PATH', ''));
const resourcePath = normalizePath(optionalEnv('MACROFACTOR_CONNECTOR_RESOURCE_PATH', joinPath(publicPath, '/mcp')));
const registrationPath = joinPath(publicPath, '/register');
const authorizationPath = joinPath(publicPath, '/authorize');
const tokenPath = joinPath(publicPath, '/token');
const issuerUrl = publicPath === '/' ? publicBaseUrl : `${publicBaseUrl}${publicPath}`;
const resourceUrl = `${publicBaseUrl}${resourcePath}`;
const protectedResourceMetadataPath = `/.well-known/oauth-protected-resource${resourcePath}`;
const authorizationServerMetadataPath =
  publicPath === '/'
    ? '/.well-known/oauth-authorization-server'
    : `/.well-known/oauth-authorization-server${publicPath}`;
const openIdConfigurationPath =
  publicPath === '/' ? '/.well-known/openid-configuration' : `/.well-known/openid-configuration${publicPath}`;
const loginSecret = requiredEnv('MACROFACTOR_CONNECTOR_LOGIN_SECRET');
const tokenSecret = optionalEnv('MACROFACTOR_CONNECTOR_TOKEN_SECRET', loginSecret);
const host = optionalEnv('MACROFACTOR_CONNECTOR_HOST', '127.0.0.1');
const port = Number(optionalEnv('MACROFACTOR_CONNECTOR_PORT', '3010'));
const upstreamUrl = optionalEnv('MACROFACTOR_MCP_UPSTREAM_URL', 'http://127.0.0.1:3001/mcp');
const upstreamToken = process.env.MACROFACTOR_MCP_UPSTREAM_TOKEN ?? process.env.MCP_AUTH_TOKEN;
const stateFilePath = resolve(
  optionalEnv('MACROFACTOR_CONNECTOR_STATE_FILE', `${homedir()}/.macrofactor-mcp-connector/state.json`)
);
const allowedRedirectUris = optionalEnv(
...
function loadState(): StateFile {
  ...
  try {
    const parsed = JSON.parse(readFileSync(stateFilePath, 'utf8')) as Partial<StateFile>;
    return {
...
function verifyToken(token: string | undefined, expectedType: TokenPayload['typ']): TokenPayload | null {
  if (!token) {
    return null;
  }
  ...
  try {
    ...
    if (
      ...
      payload.iss !== issuerUrl ||
      payload.aud !== resourceUrl ||
      payload.exp <= now ||
      !payload.scope.split(/\s+/).includes(SCOPE)
    ) {
      return null;
    }
    return payload;
...
function constantTimeSecretMatches(input: string): boolean {
...
function authChallengeHeader(): string {
  return (
...
function sendAuthChallenge(res: ServerResponse): void {
  setCorsHeaders(res);
  res.statusCode = 401;
...
function extractBearer(headers: IncomingHttpHeaders): string | undefined {
...
function redirectUriMatches(registered: string, actual: string): boolean {
  ...
  try {
    ...
    return (
      loopback.has(registeredUrl.hostname) &&
      registeredUrl.hostname === actualUrl.hostname &&
      registeredUrl.protocol === actualUrl.protocol &&
      registeredUrl.pathname === actualUrl.pathname
    );
  } catch {
    return false;
  }
...
function redirectUriAllowedByPolicy(uri: string): boolean {
  return allowedRedirectUris.some((allowed) => redirectUriMatches(allowed, uri));
}

function safeClientLabel(clientId: string): string {
...
async function clientFromId(clientId: string): Promise<RegisteredClient | null> {
  ...
  const metadata = (await response.json()) as Partial<RegisteredClient>;
...
function redirectAllowedForClient(client: RegisteredClient, redirectUri: string): boolean {
  return client.redirect_uris.some((registered) => redirectUriMatches(registered, redirectUri));
}

async function handleRegistration(req: IncomingMessage, res: ServerResponse): Promise<void> {
  if (req.method !== 'POST') {
    res.statusCode = 405;
...
function authForm(query: URLSearchParams, error?: string): string {
  const hiddenInputs = [...query.entries()]
...
function tooManyFailedAttempts(req: IncomingMessage): boolean {
...
function recordFailedAttempt(req: IncomingMessage): void {
  const ip = clientIp(req);
...
function clearFailedAttempts(req: IncomingMessage): void {
...
async function handleToken(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (grantType === 'authorization_code') {
    ...
    if (
      ...
      sha256Base64Url(codeVerifier) !== record.code_challenge
    ) {
  ...
  if (grantType === 'refresh_token') {
    ...
    const payload = verifyToken(refreshToken ?? undefined, 'refresh');
    if (!payload) {
    ...
    if (!record || record.client_id !== payload.client_id || record.expires_at <= Math.floor(Date.now() / 1000)) {
...
function mcpBodyRequiresAuth(body: unknown): boolean {
  const messages = Array.isArray(body) ? body : [body];
  for (const message of messages) {
    if (!message || typeof message !== 'object') {
      return true;
...
async function pipeUpstreamResponse(upstreamResponse: Response, res: ServerResponse): Promise<void> {
  setCorsHeaders(res);
...
async function readUpstreamText(upstreamResponse: Response): Promise<string> {
  try {
...
function summarizeUpstreamErrorBody(bodyText: string): string {
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object') {
      ...
      if (error && typeof error === 'object') {
        const message = (error as { message?: unknown; error_description?: unknown }).message;
        const description = (error as { message?: unknown; error_description?: unknown }).error_description;
    ...
    // Plain text responses are summarized below.
  }
...
async function handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (
    ...
    url.pathname === openIdConfigurationPath
  ) {
