...
| `get_training_program` | `{id?: string}`                        | Specific training program by id, or active program when id is omitted: days, exercises per day, periodization.                                           |
...
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ----------- |
| `log_food`           | `{foodId?: string, query?: string, grams?: number, amount?: number, unit?: string, servingIndex?: number, quantity?: number, date?: string, hour?: number, minute?: number}`                             | Log a catalog food by precise `foodId` or query fallback. Grams, unit mode, and exact serving-index modes are supported. | No          |
...
| `update_workout_set` | `{workoutId: string, exerciseInstanceId: string, setIndex: number, reps?: number, lbs?: number, kg?: number, rir?: number, rest?: number, type?: string, isSkipped?: boolean}`                           | Correct one logged set inside an existing workout.                                                                       | No          |
...
| `delete_food`        | `{date: string, entryId: string}`                                                                                                                                                                        | Delete a food entry by removing it from the day's food log.                                                              | **Yes**     |
